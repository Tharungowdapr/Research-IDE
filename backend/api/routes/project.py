...
async def update_input(
    ...
    """Update the research description and optionally the title."""
    project = _get_project_or_404(project_id, current_user.id, db)
    project.input_text = body.input_text
    if body.title:
        project.title = body.title
    # Clear all outputs so user re-runs from scratch
    from models.project import Output
    db.query(Output).filter(Output.project_id == project_id).delete()
    project.current_stage = "input"
    project.status = "created"
    db.commit()
    return {"message": "Input updated", "project": _project_response(project)}


@router.get("/{project_id}/history/{output_type}")
async def get_output_history(
    ...
    """Get version history for a specific output type."""
    _get_project_or_404(project_id, current_user.id, db)
    from models.project import OutputHistory
    history = (
        db.query(OutputHistory)
        .filter(OutputHistory.project_id == project_id, OutputHistory.output_type == output_type)
        .order_by(OutputHistory.created_at.desc())
        .limit(10)
        .all()
    )
    return [
        {"id": h.id, "version": h.version, "created_at": h.created_at.isoformat(), "data": h.data}
        for h in history
    ]
