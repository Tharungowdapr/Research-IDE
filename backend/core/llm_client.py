...
PROVIDER_MODELS = {
    ...
    LLMProvider.GROQ: [
        ...
        {"id": "llama-3.3-70b-versatile", "name": "Llama 3.3 70B (Latest)", "context": "128k"},
...
class LLMClient:
    ...
    async def _openai_complete(
        ...
        if not self.api_key and self.provider != LLMProvider.OLLAMA:
            raise ValueError(f"{self.provider.value} requires an API key. Add it in Settings → AI Settings.")
        headers = {
    ...
    async def _anthropic_complete(
        ...
        if not self.api_key:
            raise ValueError("Anthropic requires an API key. Add it in Settings → AI Settings → Anthropic.")
        headers = {
    ...
    async def _gemini_complete(
        ...
        if not self.api_key:
            raise ValueError("Gemini requires an API key. Add it in Settings → AI Settings → Google Gemini.")
    ...
    async def _cohere_complete(
        ...
        if not self.api_key:
            raise ValueError("Cohere requires an API key. Add it in Settings → AI Settings → Cohere.")
        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json",
        }
